...
void startLevel(short oldLevelNumber, short stairDirection) {
    ...
    if (!levels[rogue.depthLevel - 1].visited) { // level has not already been visited
        ...
        // Set aside anything that fell here from the level above.
        // Make each digDungeon() attempt on an empty slate and restore the fallen entities
        // only when level gen succeeds. This makes it possible to cleanly discard stale items
        // that would otherwise show up as phantom entries in the seed catalog.
        creatureList fallenMonsters = *monsters;
        item *fallenItems = floorItems->nextItem;
        *monsters = createCreatureList();
        ...
        for (failsafe = 50; failsafe; failsafe--) {
            ...
            // Discard the items and captive monsters generated by this attempt
        ...
        // Restore the entities that fell from the level above
        for (creatureListNode *node = fallenMonsters.head, *next; node != NULL; node = next) {
            next = node->nextCreature;
            node->nextCreature = monsters->head;
            monsters->head = node;
        }
        if (fallenItems != NULL) {
            item *tail = floorItems;
            while (tail->nextItem != NULL) {
                tail = tail->nextItem;
            }
            tail->nextItem = fallenItems;
        }
